...
| Return on Invested Capital (ROIC) | FMP `key-metrics-ttm` → `roicTTM` (server-rendered) | ✅ **Free (Basic)** — ~87 symbols only | Company page → "Statistics (TTM)" → Profitability — `https://alphascope.trade/dashboard/company/AAPL` | "ROIC" row in the "Margins & Growth" group of `StockFundamentalsStrip.tsx`, from `metrics.roicTTM` (FMP's decimal fraction normalized to percent units server-side via `fmpToPercent`) — `http://localhost:3000/stock/MSFT` | ✅ Yes |
