 ...
 */
export function aggregateSectorHeatmap(rows, days, options) {
    ...
    const todayIsWeekend = todayIso === null
        ? false
        : (() => {
            try {
    ...
    const isPartialFor = (date) => todayIso !== null && date === axisLast && date === todayIso && !todayIsWeekend;
    ...
    const allowSet = options?.allowedSectors && options.allowedSectors.length > 0
        ? new Set(options.allowedSectors.map((s) => s.trim()).filter(Boolean))
        : null;
    const acc = new Map();
    const untagged = [];
    for (const p of pipelines) {
        const effectiveSector = p.sectorRaw !== null && (allowSet === null || allowSet.has(p.sectorRaw))
            ? p.sectorRaw
            : null;
...
//# sourceMappingURL=aggregateSectorHeatmap.js.map