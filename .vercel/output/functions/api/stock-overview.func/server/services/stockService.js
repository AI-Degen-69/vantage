...
import { aggregateSectorHeatmap } from '../../shared/aggregateSectorHeatmap';
...
function fmpUrl(endpoint, params = {}) {
    const qs = new URLSearchParams({ apikey: FMP_KEY, ...params }).toString();
 ...
 */
function tickerUrl(name, symbol, extra = {}) {
    if (QUOTE_USE_QUERY_PARAM) {
        const qs = new URLSearchParams({ apikey: FMP_KEY, symbol, ...extra }).toString();
        return `${FMP_BASE}/${name}?${qs}`;
    }
    const qs = new URLSearchParams({ apikey: FMP_KEY, ...extra }).toString();
 ...
 */
function normalizeIncomeRow(raw) {
 ...
 */
function normalizeChartPoint(raw) {
    return {
        date: String(raw.date ?? ''),
...
async function yahooChart(symbol) {
    try {
        ...
        const historical = rows.map((r) => ({
            date: String(r.date instanceof Date ? r.date.toISOString().slice(0, 10) : r.date ?? ""),
...
export const stockService = {
    ...
    async getQuote(symbol) {
        ...
        if (!result && hasFmp()) {
            ...
            const url = QUOTE_USE_QUERY_PARAM
                ? fmpUrl('quote', { symbol })
                : fmpUrl(`quote/${symbol}`);
            const raw = await fetchJSON(url, `quote/${symbol}`);
            if (raw) {
        ...
        if (!result && AV_KEY) {
            ...
            const raw = await fetchJSON(url, `av quote/${symbol}`);
    ...
    async getBatchQuotes(symbols) {
        ...
        if (hasFmp() && !QUOTE_USE_QUERY_PARAM) {
            const raw = await fetchJSON(fmpUrl(`quote/${ordered.join(',')}`), `batch quote: ${ordered.length}`, 18000);
    ...
    async getProfile(symbol) {
        ...
        if (!hasFmp())
            return null;
        const raw = await fetchJSON(tickerUrl('profile', symbol), `profile/${symbol}`);
        const row = Array.isArray(raw) ? raw[0] : raw;
    ...
    async getFinancialStatements(symbol) {
        ...
        const [incomeRaw, balanceRaw, cashRaw] = await Promise.all([
            fetchJSON(tickerUrl('income-statement', symbol, { limit: 10 }), `income/${symbol}`),
            fetchJSON(tickerUrl('balance-sheet-statement', symbol, { limit: 10 }), `balance/${symbol}`),
            fetchJSON(tickerUrl('cash-flow-statement', symbol, { limit: 10 }), `cash/${symbol}`),
        ]);
        const result = {
    ...
    async getMetrics(symbol) {
        ...
        if (cache.has(cacheKey))
            return cache.get(cacheKey);
        if (!hasFmp())
            return { metrics: {}, ratios: {}, scores: null };
        const [m, r, s] = await Promise.all([
            fetchJSON(tickerUrl('key-metrics-ttm', symbol), `metrics/${symbol}`),
            fetchJSON(tickerUrl('ratios-ttm', symbol), `ratios/${symbol}`),
            fetchJSON(tickerUrl('financial-scores', symbol), `scores/${symbol}`),
        ]);
    ...
    async getInsiderTrading(symbol) {
        ...
        if (cache.has(cacheKey))
            return cache.get(cacheKey);
        try {
            const raw = await yahooFinance.quoteSummary(symbol, { modules: ['insiderTransactions'] });
            const txs = raw?.insiderTransactions?.transactions ?? [];
            const normalized = txs.map((t) => normalizeInsider(t));
            cache.set(cacheKey, normalized);
            return normalized;
        }
        catch (e) {
    ...
    async getNews(symbol) {
        ...
        if (cache.has(cacheKey))
            return cache.get(cacheKey);
        try {
            const raw = await yahooFinance.search(symbol, { newsCount: 5 });
    ...
    async getEarningsCalendar(from, to) {
        ...
        if (!hasFmp())
            return [];
        const raw = await fetchJSON(fmpUrl(EARNINGS_ENDPOINT, { from, to }), `earnings ${from}..${to}`);
    ...
    async getChart(symbol) {
        ...
        if (hasFmp()) {
            ...
            const raw = await fetchJSON(url, `chart/${symbol}`);
            if (raw) {
    ...
    async getIndexQuotes() {
        ...
        if (hasFmp() && (!out.sp500?.symbol || !out.dow?.symbol || !out.nasdaq?.symbol)) {
            try {
                const raw = await fetchJSON(fmpUrl('quote', { symbol: '^GSPC,^IXIC,^DJI' }), 'index quotes (fmp fallback)', 10000);
                const arr = Array.isArray(raw) ? raw : [];
                const find = (suffix) => {
                    const match = arr.find((r) => String(r.symbol ?? '').endsWith(suffix));
     ...
     */
    getInsightsTab(tab) {
        const validKey = Object.keys(insightsTabUniverses).includes(tab)
            ? tab
            : 'sp500';
     ...
     */
    async getFxRates(currencies = ['USD', 'ILS', 'EUR']) {
        ...
        const settled = await Promise.all(pairs.map(async (sym) => {
            try {
                ...
                return Number.isFinite(px) && px > 0 ? [sym.replace('=X', ''), px] : null;
            }
            catch {
                return null;
            }
     ...
     */
    async getSectorHeatmap(symbols, days = 5, sectorAllow = null) {
        if (symbols.length === 0) {
            return { days: [], rows: [], untagged: [], generatedAt: new Date().toISOString() };
        }
        const sortedSyms = symbols.slice().map((s) => s.toUpperCase()).sort();
        const allowKey = sectorAllow && sectorAllow.length > 0 ? sectorAllow.slice().sort().join(',') : '*';
     ...
     */
    async getSmaDistancesFor(symbols, windowSize = 200) {
        if (symbols.length === 0)
            return { rows: [] };
        ...
        const rows = await Promise.all(symbols.map(async (sym) => {
            try {
                ...
                const historical = [...chart.historical].sort((a, b) => a.date < b.date ? -1 : a.date > b.date ? 1 : 0);
