...
export const handleStockQuote = async (req, res) => {
...
export const handleBatchQuotes = async (req, res) => {
    ...
    const result = await stockService.getBatchQuotes(symbols);
    res.json(result);
};
/**
 ...
 */
export const handleStockProfile = async (req, res) => {
    const symbol = String(req.query.symbol || "");
    if (!symbol)
        return res.status(400).json({ error: "symbol parameter required" });
...
export const handleStockOverview = async (req, res) => {
    const symbol = String(req.query.symbol || "");
    if (!symbol)
        return res.status(400).json({ error: "symbol parameter required" });
    const profile = await stockService.getProfile(symbol);
...
export const handleStockFinancials = async (req, res) => {
    const symbol = String(req.query.symbol || "");
    if (!symbol)
        return res.status(400).json({ error: "symbol parameter required" });
    const data = await stockService.getFinancialStatements(symbol);
    res.json(data);
};
export const handleStockMetrics = async (req, res) => {
    const symbol = String(req.query.symbol || "");
    if (!symbol)
        return res.status(400).json({ error: "symbol parameter required" });
    const data = await stockService.getMetrics(symbol);
    res.json(data);
};
export const handleStockAnalyst = async (req, res) => {
    const symbol = String(req.query.symbol || "");
    if (!symbol)
        return res.status(400).json({ error: "symbol parameter required" });
...
export const handleStockInsider = async (req, res) => {
    const symbol = String(req.query.symbol || "");
    if (!symbol)
        return res.status(400).json({ error: "symbol parameter required" });
...
export const handleStockNews = async (req, res) => {
    const symbol = String(req.query.symbol || "");
    if (!symbol)
        return res.status(400).json({ error: "symbol parameter required" });
...
export const handleEarningsCalendar = async (req, res) => {
    const from = String(req.query.from || "");
    const to = String(req.query.to || "");
    if (!from || !to)
        return res.status(400).json({ error: "from and to parameters required" });
    const data = await stockService.getEarningsCalendar(from, to);
    res.json(data);
};
export const handleStockChart = async (req, res) => {
    const symbol = String(req.query.symbol || "");
    if (!symbol)
        return res.status(400).json({ error: "symbol parameter required" });
    const data = await stockService.getChart(symbol);
    res.json(data);
};
export const handleIndexQuotes = async (_req, res) => {
    const data = await stockService.getIndexQuotes();
    // Wrap in a typed shape for the client; missing entries are null.
    const wrap = (q, name) => (q ? { symbol: q.symbol, name, price: q.price, change: q.change, changesPercentage: q.changesPercentage } : null);
    res.json({
 ...
 */
export const handleSectorHeatmap = async (req, res) => {
    const symbolsRaw = String(req.query.symbols || "");
    if (!symbolsRaw)
        return res.status(400).json({ error: "symbols parameter required" });
    const symbols = symbolsRaw.split(",").map((s) => s.trim()).filter(Boolean);
    ...
    const sectorAllow = sectorsRaw.length > 0
        ? sectorsRaw.split(",").map((s) => s.trim()).filter(Boolean)
        : null;
    const data = await stockService.getSectorHeatmap(symbols, days, sectorAllow);
    res.json(data);
};
/**
 ...
 */
export const handleInsightsTab = async (req, res) => {
    const tab = String(req.query.tab || "sp500");
    const data = stockService.getInsightsTab(tab);
    res.json(data);
};
/**
 ...
 */
export const handleSmaDistances = async (req, res) => {
    ...
    const data = await stockService.getSmaDistancesFor(symbols, windowSize);
    res.json(data);
};
/**
