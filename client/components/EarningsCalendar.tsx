...
const EventCard = ({ ev, t, isFocus }: EventCardProps) => (
  <div
    ...
    className={`bg-secondary/30 rounded-lg p-3 border transition-colors ${
      isFocus
  ...
  >
    <div className="flex justify-between items-start mb-2">
      ...
      <div
        className={`p-1.5 rounded-md ${
          ev.time === "Before Open" ? "bg-amber-500/15 text-amber-400" : "bg-primary/15 text-primary"
        }`}
    ...
    <div className="space-y-1 text-xs">
      <div className="flex justify-between text-muted-foreground">
        ...
        <span className="font-mono tabular-nums" dir="ltr">${ev.epsEst.toFixed(2)}</span>
      </div>
      {ev.epsActual !== undefined && ev.epsActual !== null && (
        <div className="flex justify-between font-medium">
          <span>{t("earningsCalendar.actual")}</span>
          <span
            className={`font-mono tabular-nums ${ev.surprise === "beat" ? "text-chart-positive" : ev.surprise === "miss" ? "text-chart-negative" : "text-foreground"}`}
            dir="ltr"
          >
      ...
      <div className="flex justify-between text-muted-foreground pt-1">
        ...
        <span className="font-mono tabular-nums" dir="ltr">${ev.revEst.toFixed(2)}B</span>
      </div>
    </div>
  </div>
);
  ...
  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden relative">
      <div className="absolute top-2 right-2 flex items-center gap-2 z-10">
        {!isLoading && (
          <span className="text-xs text-muted-foreground bg-muted/60 px-2 py-1 rounded">
