...
export type LogoSize = LogoDevSize;
...
const sizeMap: Record<LogoSize, { box: string; text: string; rounded: string }> = {
  sm: { box: "w-8 h-8", text: "text-[10px]", rounded: "rounded" },
 ...
 *                      render initials and wait for `useStockProfile` to
 ...
 *   5. initials      — terminal fallback; 2-character ticker initials.
 */
type Tier = "ticker" | "name_pending" | "name" | "sector" | "initials";

/**
 * Convert an internal ladder tier into the persistable cache shape.
 * Note: `name_pending` collapses to `initials` in the cache so future
 * mounts skip the effect entirely instead of re-running name_pending.
 */
...
export default function TickerLogo({
  ...
  // unconditional (a conditional `useState` initializer reads better if we
  // move the branching to a ref + effect, but useState's initializer form
  // keeps things declarative and avoids a one-render flash of "ticker").
  ...
  useEffect(() => {
    ...
    if (!cached) {
      // New mount, no cache → start the ladder at tier 1.
      if (tier !== "ticker") setTier("ticker");
      return;
    }
    switch (cached.tier) {
      case "ticker":
    ...
    // `tier` intentionally NOT in deps — this is a one-time cache read on
    // mount, not a re-resolution on every tier change.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [upper, lang]);

  // ── Render-side src resolution ───────────────────────────────────────
  const tickerUrl = useMemo(() => (tier === "ticker" ? getLogoDevUrl(upper, size) : null), [tier, upper, size]);
  ...
  const nameUrl = useMemo(() => {
    ...
    const companyName = profile.data?.companyName ?? cachedNameRef.current ?? null;
  ...
  }, [tier, profile.isLoading, profile.data?.companyName, lang, upper]);

  // ── onError handlers ────────────────────────────────────────────────
  const onTickerError = useCallback(() => {
    ...
    setTier(next);
    setCachedTier(upper, toCachedTier(next, profile.data?.companyName));
  }, [tier, profile.data?.companyName, profile.isLoading, profile.data, lang, upper]);

  const onNameError = useCallback(() => {
    if (tier !== "name") return;
    const next: Tier = lang === "he" ? "sector" : "initials";
    setTier(next);
    setCachedTier(upper, toCachedTier(next, profile.data?.companyName));
  }, [tier, profile.data?.companyName, lang, upper]);

  // ── onLoad success: cache the win so the next mount skips reprobes ───
  const onTickerLoad = useCallback(() => {
    setImageLoaded(true);
    if (tier === "ticker") setCachedTier(upper, { tier: "ticker" });
  }, [tier, upper]);

  const onNameLoad = useCallback(() => {
    setImageLoaded(true);
    if (tier === "name") {
      const companyName = profile.data?.companyName ?? cachedNameRef.current ?? null;
      if (companyName) setCachedTier(upper, { tier: "name", companyName });
    }
  }, [tier, profile.data?.companyName, upper]);
  ...
  const showInitials =
    tier === "initials" ||
    (!showTickerImg && !showNameImg && !showSectorGlyph);
  ...
  return (
    ...
    >
      {/* Initials layer — visible by default, fades under a loaded image.
          ...
          transparent-background logo. Without the opacity toggle, logos
          that don't fully cover the tile would render the text faintly
          underneath. */}
      <span
